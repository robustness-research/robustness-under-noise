...
## Generate individual plots for LaTeX table

Create individual plots for specific dataset-method combinations

```{r include=FALSE}
# Define the combinations we want to plot
plot_combinations <- list(
  list(dataset = "wilt", method = "gcvEarth", label = "wilt-gcvEarth"),
  list(dataset = "diabetes", method = "ctree", label = "diabetes-ctree"),
  list(dataset = "mfeat-karhunen", method = "rf", label = "mfeat-karhunen-rf"),
  list(dataset = "ozone-level-8hr", method = "mlpML", label = "ozone-level-8hr-mlpML"),
  list(dataset = "ozone-level-8hr", method = "multinom", label = "ozone-level-8hr-multinom")
)

# Function to create and save individual plot
create_individual_plot <- function(dataset_name, method_name, label) {
  # Filter data for specific dataset and method
  plot_data <- deciles_df %>%
    filter(dataset_name == !!dataset_name, technique == !!method_name)
  
  # Create plot
  p <- ggplot(plot_data, aes(x = percentage, y = kappa_loss, color = factor(noise))) +
    geom_point(size = 1.5) +
    geom_line(aes(group = factor(noise)), linewidth = 0.8) +
    labs(x = "Instances (%)", y = "Kappa Loss", color = "Noise (%)") +
  ...
  # Save plot
  filename <- paste0("../results/plots/individual/", label, ".pdf")
  ggsave(filename, p, width = 3, height = 2.5, units = "in")
  
  return(p)
}

# Create directory if it doesn't exist
dir.create("../results/plots/individual", showWarnings = FALSE, recursive = TRUE)

# Generate all plots
for (combo in plot_combinations) {
  create_individual_plot(combo$dataset, combo$method, combo$label)
}
```
