...
print(file.exists(instances_file_candidates))
```
...
We define kappa loss as: baseline_kappa - kappa, where baseline_kappa is the kappa measured with noise == 0 for the same dataset, fold and method. If a noise==0 value cannot be found for a method/fold, the code will fall back to the maximum kappa observed for that method/fold.
...
df_kappa2 %>% filter(noise == 0) %>% select(dataset, fold, method, percent, kappa, baseline_kappa, kappa_loss) %>% head()
...
# Save
dir.create("results/instances/folds", recursive = TRUE, showWarnings = FALSE)
saveRDS(agg_by_noise, file = "results/instances/folds/agg_kappa_loss_by_noise_per_fold.rds")
...
saveRDS(agg_by_percent, file = "results/instances/folds/agg_kappa_loss_by_percent_per_fold.rds")
...
```{r stats}
dir.create("results/instances/folds/tests", recursive = TRUE, showWarnings = FALSE)
...
saveRDS(tests_by_noise, file = "results/instances/folds/tests/tests_by_noise_per_fold.rds")
...
saveRDS(tests_by_percent, file = "results/instances/folds/tests/tests_by_percent_per_fold.rds")
...
if (nrow(sum_noise)>0) write.csv(sum_noise, file = "results/instances/folds/tests/summary_tests_by_noise.csv", row.names = FALSE)
if (nrow(sum_percent)>0) write.csv(sum_percent, file = "results/instances/folds/tests/summary_tests_by_percent.csv", row.names = FALSE)

cat("Statistical tests completed. Results saved to results/instances/folds/tests/\n")

```

## Notes

- This report assumes the nested structure dataset -> fold -> method -> noise -> percent -> confusionMatrix (or at least an object that contains an `overall["Kappa"]` entry). If your stored object name differs, please point me to the correct file or structure and I will adapt the extractor.
- The two saved aggregate files are:
  - `results/instances/folds/agg_kappa_loss_by_noise_per_fold.rds` (and .csv)
  - `results/instances/folds/agg_kappa_loss_by_percent_per_fold.rds` (and .csv)
- The statistical test outputs are saved under `results/instances/folds/tests/` as RDS and short CSV summaries.

You can now knit this Rmd to produce an HTML with the tables, statistical test summaries and the saved CSVs for downstream plotting/analysis.
