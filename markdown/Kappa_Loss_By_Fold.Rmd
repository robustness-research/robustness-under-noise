title: "Kappa loss per fold (by noise level and by percent altered)" output: html_document ---
...
```{r plot-noise, fig.width=20, fig.height=16}
library(ggplot2)
...
```{r plot-percent, fig.width=20, fig.height=16}
...
summarize_test <- function(tlist) {
  ...
  for (nm in names(tlist)) {
    ...
    if (!is.null(res$pairwise_wilcox) && inherits(res$pairwise_wilcox, "pairwise.htest") && !inherits(res$pairwise_wilcox, "error")) {
      ...
      if (!is.null(pmat)) {
        for (i in seq_len(nrow(pmat))) for (j in seq_len(ncol(pmat))) {
          pv <- pmat[i,j]
          if (!is.na(pv)) {
            comparison <- paste(rownames(pmat)[i], colnames(pmat)[j], sep = " vs ")
            rows[[ri]] <- data.frame(target = nm, test = "pairwise_wilcox", comparison = comparison, p.value = pv, statistic = NA, stringsAsFactors = FALSE); ri <- ri+1
          }
        }
      }
    }
...
cat("Summary of tests found:\n")
if (nrow(sum_noise) > 0) {
  cat("Noise tests:\n")
  print(table(sum_noise$test))
}
if (nrow(sum_percent) > 0) {
  cat("Percent tests:\n")
  print(table(sum_percent$test))
}
